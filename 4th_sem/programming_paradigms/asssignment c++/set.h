...
void Set::InputV(Set V)
{
	...
	for (i=0;i<n;i++)
	{
		...
		while(V.search(s)!=1)
		{
		cout<<"the starting vertex not part of vertices"<<endl;
		cin>>s;
	    }
	    cout<<"give the ending vertex"<<endl;
  	    cin>>f;
		while(V.search(f)!=1)
		{
		cout<<"the ending vertex not part of vertices"<<endl;
		cin>>f;
	    }
		set[i]=10*s+f;
