...
void *recv_cli_loop(void* network_socket)
{
   ...
   while(1)
   {
     ...
     char recv_response[256],recv_name[256];
     int recving=recv(*network,&recv_name,sizeof(recv_name),0);
     recving=recv(*network,&recv_response,sizeof(recv_response),0);
