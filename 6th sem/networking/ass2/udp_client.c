...
int main(int argc,char* argv[]){
   ...
   else
   {
      ...
      for(i=0;i<atoi(argv[5]);i++)
      {
        ...
        nBytes = recvfrom(client_socket,buffer1,1024,0,(struct sockaddr *)&server_address,addr_size);
        if (!gettimeofday(&timer_usec, NULL)) 
             {
             timestamp_usec = ((long long int) timer_usec.tv_sec) * 1000000ll +(long long int) timer_usec.tv_usec;
              }
        ...
        sprintf(time_data1,"%08lld", timestamp_usec-timestamp_usec1);
        if(verbose==1)
        {
           
           printf("Timestamp recv: %s\n",time_data1);
           printf("Received from server: %s\n",buffer1);
        }
        long long int time_1=atoi(time_data1)-atoi(time_data);
        sprintf(time_data1,"%08lld", time_1);
        printf("Round Trip Time: %s\n",time_data1);
        sum+=time_1;
        


      }
      printf("Avg Round Trip Time: %lld\n",sum/atoi(argv[5]));
      close(client_socket);
